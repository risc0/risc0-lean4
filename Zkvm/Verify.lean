...
def verify (Elem ExtElem: Type) [Monad M] [Monad.MonadVerify Elem ExtElem M] [PrimeField Elem] (journal: Subarray UInt32)
  : M Unit
  := do MonadVerifyAdapter.execute
        MonadVerifyAdapter.verifyOutput journal
        throw (VerificationError.Sorry "Zkvm.Verify.verify")

def run_verify [PrimeField Elem] (circuit: Circuit Elem ExtElem) (journal seal: Subarray UInt32)
